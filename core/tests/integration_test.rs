...
fn testdata_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("testdata")
}
...
fn test_end_to_end_scan_search_metadata() {
    ...
    assert!(stacks.len() >= 4, "Expected at least 4 stacks, found {}", stacks.len());
    ...
    metadata.xmp_tags.insert("description".to_string(), "Family photo 2024".to_string());
    metadata.custom_tags.insert("ocr_text".to_string(), serde_json::json!("Reunion 2024"));
    
    repo.write_metadata(&stack, &metadata).unwrap();

    // Re-scan and verify metadata persists
    let stacks_after = repo.scan().unwrap();
    let stack_after = stacks_after.iter().find(|s| s.id == "FamilyPhotos_0001").unwrap();
    ...
    assert!(stack_after.metadata.xmp_tags.contains_key("description") 
        || stack_after.metadata.custom_tags.contains_key("xmp:description"));
}
...
fn test_xmp_readable_by_exif_tools() {
    ...
    // Write XMP metadata
    let mut metadata = Metadata::default();
    metadata.xmp_tags.insert("description".to_string(), "XMP test description".to_string());
    metadata.xmp_tags.insert("creator".to_string(), "Test Author".to_string());
    ...
    assert_eq!(xmp_tags.get("description"), Some(&"XMP test description".to_string()));
...
fn test_with_committed_testdata() {
    ...
    for stack in &stacks {
        if let Some(ref path) = stack.original {
            ...
            if !tags.is_empty() {
                assert!(tags.contains_key("Make") || tags.contains_key("Model"),
...
fn test_search_workflow() {
    ...
    if let Some(stack) = stacks.iter().find(|s| s.id == "FamilyPhotos_0001") {
        let mut metadata = Metadata::default();
        metadata.custom_tags.insert("ocr_text".to_string(), serde_json::json!("Birthday party"));
        repo.write_metadata(stack, &metadata).unwrap();
    }
    ...
    if let Some(stack) = stacks.iter().find(|s| s.id == "FamilyPhotos_0002") {
        let mut metadata = Metadata::default();
        metadata.custom_tags.insert("ocr_text".to_string(), serde_json::json!("Wedding ceremony"));
        repo.write_metadata(stack, &metadata).unwrap();
    }
...
fn test_tiff_workflow() {
    ...
    assert_eq!(stack.format(), Some(photostax_core::metadata::ImageFormat::Tiff));

    // Write XMP metadata (should create sidecar for TIFF)
    let mut metadata = Metadata::default();
    metadata.xmp_tags.insert("description".to_string(), "TIFF test".to_string());
    ...
    assert!(sidecar_path.exists(), "XMP sidecar should be created for TIFF");
...
fn test_mixed_format_stack() {
    ...
    assert_eq!(stack.format(), Some(photostax_core::metadata::ImageFormat::Jpeg));
}
